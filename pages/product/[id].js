...
export default function ProductPage({ product }) {
  ...
  function changeSrcLeft() {
    ...
    imageSrc === productImages[0] && productImages[2]
  ...
  function changeSrcRight() {
    ...
    imageSrc === productImages[0] && productImages[1]
    ...
    <>
      <div className={styles.product_page}>
        <div className={styles.breadcrumbs}>
          ...
          <span>/</span>
          <Link href={"/productList/" + product.category}>
        ...
        <div className={styles.right}>
            ...
            <div className={styles.input_group}>
              <>
                {product.color.length > 1 ? (
                  ...
                  <p>
                    Color:{" "}
                    {product.color[0].charAt(0).toUpperCase() +
                      product.color[0].substring(1)}
                  </p>
                )}
              </>
            </div>
          ...
          <button
            className={`primary-button ${buttonStyles.button} ${
              animate ? buttonStyles.success : null
            }`}
            ref={buyButton}
            onClick={() => {
              animateButton();
...
export async function getStaticProps(context) {
  ...
  const req = await fetch(
    "https://cocktails-240e.restdb.io/rest/disc-connection/" + id,
    {
      method: "GET",
      headers: {
        "cache-control": "no-cache",
